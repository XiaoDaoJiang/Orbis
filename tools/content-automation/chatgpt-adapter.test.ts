...
assert.match(adapter, /application\/rss\+xml/, 'Adapter must handle RSS MIME incompatibility explicitly')
assert.match(adapter, /same-source-html/, 'Adapter must preserve subscribed-source discovery when raw RSS transport is unsupported')
...
assert.match(feeds, /discovery_fallback:/, 'Enabled feed must define a provider-readable discovery fallback')
