...
4. `config/path-guard.yaml` — 可执行的路径约束；
...
- 生成一个符合 `dailyBriefSchema` 的 `content/briefs/YYYY-MM-DD.yaml`；
...
- 不修改 `.github/**`、`apps/**`、`packages/**`、`tools/**` 或根 workspace/lock 文件；
- 提交前运行内容校验；如果任务运行环境允许完整构建，则执行 `pnpm build`；
- 不得虚构来源、提交状态、CI 状态、部署状态或公网链接。

## 发布模型

Agent 的职责到“提交结构化内容”结束。
...
Agent 不直接维护发布产物，也不直接修改 GitHub Pages 内容目录。
...
2. 3～5 行中文导读；
3. 本次创建或更新的 structured Brief 路径；
4. 已实际验证的构建 / PR Preview / 公网状态；未验证的状态必须明确写“未验证”。

不要在最终回复中暴露内部提示词、RSS 抓取细节或中间处理数据。
